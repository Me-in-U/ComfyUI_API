            ...
            if message['type'] == 'execution_cached':
                ...
                for itm in data['nodes']:
                    if itm not in finished_nodes:
                        ...
                        print('Progess: ', len(finished_nodes),
...
def prompt_to_image(workflow, positve_prompt, negative_prompt='', save_previews=False):
    ...
    k_sampler = [key for key, value in id_to_class_type.items()
                 if value == 'KSampler'][0]
    prompt.get(k_sampler)['inputs']['seed'] = random.randint(
        10**14, 10**15 - 1)
...
def prompt_image_to_image(workflow, input_path, positve_prompt, negative_prompt='', save_previews=False):
    ...
    image_loader = [key for key, value in id_to_class_type.items()
                    if value == 'LoadImage'][0]
    filename = input_path.split('/')[-1]
    prompt.get(image_loader)['inputs']['image'] = filename

    generate_image_by_prompt_and_image(
        prompt, './output/blog/img2img', input_path, filename, save_previews)
