...
from api.websocket_api import get_history, get_image, queue_prompt, upload_image


def generate_image_by_prompt_and_image(
    prompt, output_path, input_path, filename, save_previews=False
):
    # * 사람 + 프롬프트 => 이미지
    try:
...
def generate_image_by_image_image(
    prompt,
    output_path,
    input_path1,
    filename1,
    ...
    try:
        ...
        upload_image(input_path2, filename2, server_address)
        prompt_id = queue_prompt(prompt, client_id, server_address)["prompt_id"]
        track_progress(prompt, ws, prompt_id)
        images = get_images(prompt_id, server_address, save_previews)
        save_image(images, output_path, save_previews)
    ...
    with open("tempImage.txt", "a") as log_file:
        for itm in images:
            ...
            try:
                ...
                print(f"Successfully saved image {itm['file_name']} in {directory}")
                ...
                print(f"IOError - failed to save image {itm['file_name']}: {io_err}")
            except Exception as e:
